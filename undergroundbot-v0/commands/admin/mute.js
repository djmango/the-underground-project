...
module.exports = class SayCommand extends Command {
  ...
  async run(msg, args) {
    ...
    cooldownCheck(msg.author.id, "mute", function(result) {
      ...
      if (!time) return msg.reply("Thats not a number, dimwit");
      if (!mentions)
        return msg.reply(
      ...
      adminCheck(msg.author.id, function(result) {
        if (result == true) {
          adminCheck(mentions.id, async function(result) {
            if (result == true)
              return msg.reply(`${mentions.username} is an admin!`);
            else {
              ...
              db.exec(
                `insert into commandQueue values ('unmute', ${
                  command.executeTime
                }, ${command.data})`
              );
              ...
              return msg.reply(
                `Muted ${mentions.username} for ${time} minutes!`
              );
            }
          });
